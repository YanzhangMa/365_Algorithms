...
# select the triangle that has a minimum number of adjacent non-strip triangles
def helper_Initial(tri):
    # record the number of adjacent non-strip triangles
    non_strip = [5] * len(tri)
    for index, triangle in enumerate(tri):
        if triangle.nextTri == None and triangle.prevTri == None:
            counter = 0
            for adj_triangle in triangle.adjTris:
                if adj_triangle.nextTri == None and adj_triangle.prevTri == None:
                    counter += 1
            non_strip[index] = counter
    # find the min of adjacent non-strip triangles
    minmum_nonadj = min(non_strip)
    initial_index = -1
    if minmum_nonadj == 5:
        return initial_index
    # find the index of min and the index is not in the skipping index
    for index in range(len(tri)):
        if non_strip[index] == minmum_nonadj:
            initial_index = index
            break
    return initial_index


def helper_adjacent(tri):
    adj_tri = None
    min_num = 4
    for triangle in tri.adjTris:
        if triangle.nextTri == None and triangle.prevTri == None:
            counter = 0
            for adj_triangle in triangle.adjTris:
                if adj_triangle.nextTri == None and adj_triangle.prevTri == None:
...
def buildTristrips(triangles):
    ...
    index = helper_Initial(triangles)

    while index != -1:

        # get the triangle
        initial_tri = triangles[index]

        # current should point to start in the beginning
        current_tri = initial_tri
        while True:
            next_tri = helper_adjacent(current_tri)
            if next_tri == None:
                count += 1
                if current_tri == initial_tri:
                    current_tri.prevTri = current_tri
                break

...
def buildTriangles( slice0, slice1 ):
    ...
    # brute force to find the index for the closest pair of vertices
    for a, s0_vert in enumerate(slice0.verts):
        for b, s1_vert in enumerate(slice1.verts):
            tmp_distance = length(subtract(s0_vert.coords, s1_vert.coords))
            if miniDist > tmp_distance:
                miniDist = tmp_distance
                vert_index0 = a
                vert_index1 = b
    ...
    # conduct cyclic permutation
    tmp_verts = []
    for i, vert in enumerate(slice0.verts):
        tmp_verts.append(slice0.verts[i-vert_index0-1])

    # add first to the end 
    tmp_verts.append(tmp_verts[0])
    slice0.verts = tmp_verts

    tmp_verts = []
    for i, vert in enumerate(slice1.verts):
        tmp_verts.append(slice1.verts[i-vert_index1-1])
    tmp_verts.append(tmp_verts[0])
    slice1.verts = tmp_verts
    ...
    # [YOUR CODE HERE]
    # use vertices in slice 1 as rows and vertices in slice0 as columns
    min_Area = [[0 for i in range(0, len(slice0.verts))] for j in range(0, len(slice1.verts))]
    min_Dir = [[0 for i in range(0, len(slice0.verts))] for j in range(0, len(slice1.verts))] 
    ...
    # [2 marks]
    # [YOUR CODE HERE]
    cols = len(slice0.verts)
    cur_col = 1
    # only come from previous column direction
    while cur_col < cols:
        min_Area[0][cur_col] = triangleArea(slice0.verts[cur_col-1].coords, slice0.verts[cur_col].coords, slice1.verts[0].coords) + min_Area[0][cur_col-1]
        min_Dir[0][cur_col] = 2
        cur_col += 1
    ...
    ## [YOUR CODE HERE]
    rows = len(slice1.verts)
    row = 1
    # only come from previous row direction
    while row < rows:
        min_Area[row][0] = triangleArea(slice0.verts[0].coords, slice1.verts[row-1].coords, slice1.verts[row].coords) + min_Area[row-1][0]
        min_Dir[row][0] = 1
        row += 1
    ...
    while row < rows:
        cur_col = 1
        while cur_col < cols:
            # calculate the areas from previous row and column directions
            area1 = min_Area[row][cur_col-1] + triangleArea(slice1.verts[row].coords, slice0.verts[cur_col-1].coords, slice0.verts[cur_col].coords)
            area2 = min_Area[row-1][cur_col] + triangleArea(slice1.verts[row].coords, slice1.verts[row-1].coords, slice0.verts[cur_col].coords)
            if area1 > area2:
                # previous row direction gives smaller area
                min_Area[row][cur_col] = area2
                min_Dir[row][cur_col] = 1
            else:
                # previous column direction gives smaller area
                min_Area[row][cur_col] = area1
                min_Dir[row][cur_col] = 2
            cur_col += 1
        row += 1
    ...
    while True:
        # comes from previous row
        if min_Dir[i][j] == 1:
            triangle = Triangle([slice0.verts[j], slice1.verts[i], slice1.verts[i-1]])
            triangles.append(triangle)
            i = i-1
        # comes from previous column
        else:
            triangle = Triangle([slice0.verts[j], slice0.verts[j-1], slice1.verts[i]])
            triangles.append(triangle)
            j = j-1
        if i == 0 and j == 0:
            break
